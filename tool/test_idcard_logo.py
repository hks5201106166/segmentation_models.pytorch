...
DATA_DIR = '/home/simple/mydemo/segmentation_models_mulclass/data/val'

x_test_dir = os.path.join(DATA_DIR, 'img')
y_test_dir = os.path.join(DATA_DIR, 'new_mask')
...
for i in range(l):
    ...
    t2=time.clock()
    print(t2-t1)
    pr_mask = (pr_mask.squeeze().permute(1,2,0).cpu().numpy().round())[:,:,1]
