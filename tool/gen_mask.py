...
import cv2
path='/home/simple/mydemo/ocr_project/segment/data/logolabel/train/img'
images_name=os.listdir(path)
for image_name in images_name:
    js_name=image_name.split('.')[0]+'.json'
    image=imageio.imread(os.path.join(path,image_name))
    image=image[:,:,0:3]
    f = open('/home/simple/mydemo/ocr_project/segment/data/logolabel/train/json/'+js_name, encoding='utf-8')
    res = f.read()
    js=json.loads(res)
    shapes=js['shapes']
    ...
    segmap=np.zeros((image.shape[0],image.shape[1],1),dtype=np.uint8)
    for index,label in enumerate(labels):
        segmap = polygons[index].draw_on_image(segmap,color=(255),alpha=1.0, alpha_lines=0.0, alpha_points=0.0)
    segmap=segmap/128
    # segmap = np.argmax(segmap, axis=2)
    # print(segmap.max())
    segmap=segmap.astype(np.uint8)
    imageio.imwrite('/home/simple/mydemo/ocr_project/segment/data/logolabel/train/mask_logo/'+image_name,segmap)
    # cv2.imshow('jkls',segmap)
    # cv2.waitKey(0)
    #cv2.imwrite('/home/simple/mydemo/ocr_project/segment/data/logolabel/val/mask_logo/'+image_name,segmap)
    #print(polygons)
    # for index,label in enumerate(labels):
    #     #backgourd:0
    #     #up_Obverse：1
